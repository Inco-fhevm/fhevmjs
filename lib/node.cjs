...
const handleTypes = Object.freeze({
    ebool: 0,
    euint4: 1,
    euint8: 2,
    euint16: 3,
...
function isHandleType(value) {
    return Object.values(handleTypes).includes(value);
}
function mustGetHandleType(value) {
...
function computePrehandle({ ciphertext, indexHandle, handleType, handleVersion, }) {
    ...
    const ciphertextHash = new sha3.Keccak(256)
        .update(Buffer.from(ciphertext))
        .digest();
    const ciphertextIndexHash = new sha3.Keccak(256)
...
function computeHandle({ prehandle, inputCtx, }) {
...
exports.isHandleType = isHandleType;
exports.mustGetHandleType = mustGetHandleType;
